...
class AwsInstancesSecurityGroupLoader < AwsSecurityGroupLoader
  ...
  def load_groups
    ...
    insts.each do |i|
      i.security_groups.each do |g|
        if instance_groups[g.group_id].nil?
          raise "ERROR: Security group '#{g.group_id}' on instances '#{i.id}' doesn't seem to exist in the list of all valid EC2 security groups."
        end
        instance_groups[g.group_id].add(i)
      end
    end
    ...
    insts.each do |i|
      dst_node_uid = i.id
      dst_node_name = instance_name(i)
      dst_node = add_inst_node(dst_node_uid, dst_node_name, i.vpc_id || 'classic', 'instance', i)
      dst_node.owner_id = ''
      ...
      sgs.each do |sg|
        sg.ip_permissions.each do |p|
          ...
          # If permission from an AWS group that we own, create connections to each instance that uses has that group assigned
